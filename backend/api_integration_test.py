#!/usr/bin/env python3
"""
실제 ChatGPT API와 통합된 엔드투엔드 프롬프트 테스트
...
치료비 부담을 덜어드리기 위해 선지급도 가능합니다. 전담 상담사가 자세히 안내해드릴까요? 언제든 연락 주세요 ☎️"""
    ...
    async def test_single_scenario(self, session: aiohttp.ClientSession, scenario: dict) -> dict:
        """단일 시나리오 테스트"""
    ...
    async def run_api_integration_test(self):
        """API 통합 테스트 실행"""
        ...
        async with aiohttp.ClientSession() as session:
            for scenario in self.test_scenarios:
                try:
                    result = await self.test_single_scenario(session, scenario)
...
def main():
    ...
    asyncio.run(tester.run_api_integration_test())
